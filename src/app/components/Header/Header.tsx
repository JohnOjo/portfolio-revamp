...
const headerButtons = [BIO, TECHNOLOGIES_USED, MY_WORK, ABOUT_ME];
const Header = () => {
  ...
  const renderHeaderButtons = () => {
    return headerButtons.map((headerButton) =>
  ...
  return (
    <div className={"header"}>
      <div className={"header-text-container"} onClick={() => handleClick(BIO)}>
        <div className={"header-text header-title typed-out"}>{"John Ojo"}</div>
      </div>
      <div className={"f-1"} />
      {!isDesktop && !dropdownOpen && (
        <Image
          className={"dropdown"}
          src={MobileNavDropdown}
          alt={"dropdown"}
